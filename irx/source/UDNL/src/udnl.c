...
static struct RomdirFileStat *GetFileStatFromImage(const struct RomImgData *ImageStat, const char *filename, struct RomdirFileStat *stat){
	...
	if(ImageStat->RomdirStart!=NULL){
		...
		do{
			if(((unsigned int *)filename_temp)[0]==((unsigned int*)RomdirEntry->name)[0]
				&& ((unsigned int *)filename_temp)[1]==((unsigned int*)RomdirEntry->name)[1]
				&& (*(unsigned short int*)&((unsigned int *)filename_temp)[2]==*(unsigned short int*)&((unsigned int*)RomdirEntry->name)[2])){
...
static void BeginBootupSequence(struct ResetData *ResetData){
	...
	//HALT loop
	while(1) *(volatile unsigned char *)0x80000000=2;
}
