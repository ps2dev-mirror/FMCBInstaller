...
/* 51 */ int  mcman_flushmccache(int port, int slot);
/* 52 */ int  mcman_setdirentrystate(int port, int slot, int cluster, int fsindex, int flags);
/* 53 */ int mcman_GetIOSema(void);
/* 62 */ int mcman_clearcache(int port, int slot);	//Not part of MCMAN, but required for crosslinking hacks. Invalidates the cluster cache.
